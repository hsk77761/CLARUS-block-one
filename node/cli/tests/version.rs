...
fn expected_regex() -> Regex {
    Regex::new(r"^clarus-node (.+)-([a-f\d]+)$").unwrap()
}

#[test]
fn version_is_full() {
    let expected = expected_regex();
    let output = Command::new(cargo_bin("clarus-node"))
        .args(&["--version"])
        .output()
        .unwrap();
...
fn test_regex_matches_properly() {
    ...
    let captures = expected.captures("clarus-node 2.0.0-da487d19d").unwrap();
    assert_eq!(&captures[1], "2.0.0");
    assert_eq!(&captures[2], "da487d19d");

    let captures = expected
        .captures("clarus-node 2.0.0-alpha.5-da487d19d")
        .unwrap();
